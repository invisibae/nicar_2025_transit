...
We use r5r's accessibility features to quanitfy access to opportunity
...
We'll begin making our heatmap by applying some spatial interpolation with the `interp` package.
```{r}
...
Next, we'll transform our interpolated data into a `stars` object that we'll use as the heatmap layer on our map
```{r}
...
# Add the street network as a second mapview layer
...
mapview(contour_clipped, alpha.regions = 0.75) +
mapview(baltimore_boundary, alpha.regions = .5, col.regions = "white") 
