...
osm_path <- "data/poa/2024-08-26/baltimore.osm.pbf"
```
...
-   **GTFS.zip**
    ...
    -   For this session, we've supplied Baltimore GTFS feeds for August 26, 2024 – The first day of school.

```{r}
gtfs_paths <- c("data/poa/2024-08-26/light_rail_gtfs.zip", "data/poa/metro_gtfs.zip", "data/poa/local-bus_gtfs.zip")
```
...
The amount of memory you allocate to r5r will depend on the size of the network you're working with. For small to mid-sized cities, 8GB should be sufficient. For larger cities or more difficult routing tasks, you may need to allocate more memory.
...
## What does our transit network look like?
```{r}
...
mapview(df, zcol = "mode") +
  ...
  mapview(western_departure_tiems$school_polygon[1]) +
  mapview(brooke_bourne$geometry.y)
...
Let's see what Brooke's options look like if she's willing to spend an extra 25 minutes on her commute.
```{r}
